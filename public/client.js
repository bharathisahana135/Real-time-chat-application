...
const socket = io(); 
// UI refs 
const messagesEl = document.getElementById('messages'); const composer = document.getElementById('composer'); const msgInput = document.getElementById('msgInput'); const usersEl = document.getElementById('users'); 
const typingEl = document.getElementById('typing'); 
const myNameEl = document.getElementById('myName'); 
const myAvatarEl = document.getElementById('myAvatar'); const changeNameBtn = document.getElementById('changeNameBtn'); const nameModal = document.getElementById('nameModal'); const nameInput = document.getElementById('nameInput'); const saveName = document.getElementById('saveName'); const cancelName = document.getElementById('cancelName'); 
...
changeNameBtn.addEventListener('click', () => {  nameInput.value = myName; 
...
socket.on('typing', ({ username, isTyping }) => {  if (isTyping) { 
 typingEl.textContent = `${username} is typing…`;  } else { 
...
function renderMessage(msg){ 
 ...
 el.textContent = `${formatTime(msg.ts)} — ${msg.text}`;  messagesEl.appendChild(el); 
 return; 
 } 
...
style="width:34px;height:34px;border-radius:8px;font size:13px">${(u[0]||'U').toUpperCase()}</div><div  
style="flex:1">${escapeHtml(u)}</div>`; 
